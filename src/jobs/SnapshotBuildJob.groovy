...
public class SnapshotBuildJob {
	...
	static void createJob(def dslFactory) {
		...
		def newMavenJob = dslFactory.mavenJob(Utilities.RELEASE_UNIT_NAME + '-snapshotbuild')
	...
	static void addSteps(def job) {
        job.with {
		    ...
		    description(Utilities.RELEASE_UNIT_NAME + 'snapshot build ')
			...
			preBuildSteps {
		        shell("echo 'run before Maven'")
		      	...
		      	//build.setDisplayName('#${ENV,var="RELEASE_TAG"} (${BUILD_NUMBER})')
		      	// seems not to work as an build step
		      	//wrappers {
    ...
    static void addPublisher(def job) {
        job.with {
            publishers {
		        extendedEmailPublisher { 
		            ...
		            project_recipient_list('cc:sebastian.roetus@atos.net, cc:bernhard.hirsch@accenture.com, cc:ninad.joshi@accenture.com, cc:dominik.seichter@accenture.com')
		            project_default_subject('Urgent Fix is needed! ' + Utilities.RELEASE_UNIT_NAME + ' Snapshot Build is broken!')
		            project_default_content('''Hi,
		...
		''')
		            project_content_type('text/plain')
		          	project_attachments('') 
		          	project_presend_script('''import hudson.model.*
